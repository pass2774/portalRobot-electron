...
#     __dirname__ =os.path.join(sys._MEIPASS,"..","..") # runned as a .exe file
    __dirname__ = os.path.join(os.path.dirname(os.path.abspath(sys.executable)),"..","src")
    print(".exe dirname==")
else:
...
def initDxlGroupPosControl(dxl_params):
    # Setup for position control-based dxl motors
    for idx, param in dxl_params.items():
        i = int(idx)
        ...
        # DISABLE Dynamixel#00i Operating mode
        dxl_comm_result, dxl_error = packetHandler.write1ByteTxRx(portHandler, i, ADDR_PRO_OPERATING_MODE, param["mode"])
        if dxl_comm_result != COMM_SUCCESS:
            print("%s" % packetHandler.getTxRxResult(dxl_comm_result))
        elif dxl_error != 0:
            print("%s" % packetHandler.getRxPacketError(dxl_error))
...
#         dxl_comm_result, dxl_error = packetHandler.write4ByteTxRx(portHandler, i, ADDR_PRO_PROFILE_ACC, dxl_param["pos-control"][str(i)]["profile"]["acc"])
        dxl_comm_result, dxl_error = packetHandler.write4ByteTxRx(portHandler, i, ADDR_PRO_PROFILE_ACC, param["profile"]["acc"])
        if dxl_comm_result != COMM_SUCCESS:
            print("%s" % packetHandler.getTxRxResult(dxl_comm_result))
        elif dxl_error != 0:
            print("%s" % packetHandler.getRxPacketError(dxl_error))
...
#         dxl_comm_result, dxl_error = packetHandler.write4ByteTxRx(portHandler, i, ADDR_PRO_PROFILE_VEL, dxl_param["pos-control"][str(i)]["profile"]["vel"])
        dxl_comm_result, dxl_error = packetHandler.write4ByteTxRx(portHandler, i, ADDR_PRO_PROFILE_VEL, param["profile"]["vel"])
        if dxl_comm_result != COMM_SUCCESS:
            print("%s" % packetHandler.getTxRxResult(dxl_comm_result))
        elif dxl_error != 0:
            print("%s" % packetHandler.getRxPacketError(dxl_error))
...
def dxl_SyncWrite(h_groupSyncWrite,target_state):
    print("target:",target_state)
    for idx, value in target_state.items():
        i = int(idx)
        # Allocate goal position value into byte array
        buffer = [DXL_LOBYTE(DXL_LOWORD(value)), DXL_HIBYTE(DXL_LOWORD(value)), DXL_LOBYTE(DXL_HIWORD(value)), DXL_HIBYTE(DXL_HIWORD(value))]
...
def dxl_ReadState(h_groupSyncRead,dxl_id_mask,REG_ADDR,REG_LEN):
    # Syncread present state
    dxl_comm_result = h_groupSyncRead.txRxPacket()
    if dxl_comm_result != COMM_SUCCESS:
        print("%s" % packetHandler.getTxRxResult(dxl_comm_result))
    dxl_current_state={}
    for idx in dxl_id_mask:
        i = int(idx)
        ...
        dxl_current_state[idx]=h_groupSyncRead.getData(i, REG_ADDR, REG_LEN)
    return [True, dxl_current_state]

def printPosState(dxl_id_mask):
    [isPosAvailable,dxl_current_pos]=dxl_ReadState(groupSyncReadPos,dxl_id_mask,ADDR_PRO_PRESENT_POSITION,LEN_PRO_PRESENT_POSITION)
...
if __name__ =="__main__":
    dxl_id_mask=["0","1","2","3"]
    filtered_params = {key:value for key, value in dxl_param["pos-control"].items() if key in dxl_id_mask}
    print(filtered_params)
    initDxlGroupPosControl(filtered_params)
    print("dxlRobot.py param completed.")
    [isPosAvailable,dxl_current_pos]=dxl_ReadState(groupSyncReadPos,dxl_id_mask,ADDR_PRO_PRESENT_POSITION,LEN_PRO_PRESENT_POSITION)
    if isPosAvailable == True:
        print(dxl_current_pos)

    target_state={"0":1000+dxl_current_pos["0"],"1":2000,"2":1000,"3":2000}
    dxl_SyncWrite(groupSyncWritePos,target_state)
